...
- disconn3ct (SMS) — Bambu printer camera streaming (Jan 2024), repo maintainer
...
3. `ThreadingHTTPServer.allow_reuse_address = True` so a restart on an unchanged port
   does not fail with `EADDRINUSE` while the previous socket is still in `TIME_WAIT`.
...
No other modifications. When updating to a newer upstream commit, re-apply all patches
and update the pinned commit, date, and hashes above.
