...
# OctoPrint-BambuCam patch: resolve the watermark font relative to this file
# instead of the process working directory, otherwise ImageFont.truetype raises
# "OSError: cannot open resource" depending on where webcamd was started.
...
class WebRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path.lower().startswith("/?info"):
            ...
            # OctoPrint-BambuCam patch: never expose the printer access code via /?info
            safeconfig = dict(vars(myargs))
            safeconfig["password"] = "<redacted>"
            jsonstr = ('{"stats":{"server": "%s", "encodeFps": %.2f, "sessionCount": %d, "avgStreamFps": %.2f, "sessions": %s, "snapshots": %d}, "config": %s}' % (host, self.server.getEncodeFps(), len(streamFps), fpsavg, json.dumps(streamFps) if len(streamFps) > 0 else "{}", snapshots, json.dumps(safeconfig)))
        ...
        # OctoPrint-BambuCam patch: /?shutdown disabled — lifecycle is managed by the
        # OctoPrint plugin; an unauthenticated remote shutdown endpoint is a DoS vector.
        if self.path.lower().startswith("/?shutdown"):
            self.send_response(403)
            self.send_header("Content-type", "text/html")
            self.end_headers()
            self.wfile.write(b"<html><body>shutdown endpoint disabled by OctoPrint-BambuCam</body></html>")
            return
    ...
    def streamVideo(self, rotate=-1, showFps = False):
        ...
        fpsFont = ImageFont.truetype(FONT_PATH, 14)
